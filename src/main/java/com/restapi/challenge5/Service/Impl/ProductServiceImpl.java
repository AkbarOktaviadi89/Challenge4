...
import com.restapi.challenge5.Repository.ProductRepository;
import com.restapi.challenge5.Service.ProductService;
...
public class ProductServiceImpl implements ProductService {
    ...
    public void updateById(Product product) {
        try {
            log.info("Updating product with code {}.", product.getProductCode());
            Optional<Product> existingProduct = productRepository.findById(product.getProductCode());
            if(existingProduct.isPresent()){
                productRepository.save(product);
            } else{
                log.error("Product with code {} not found. Update failed.", product.getProductCode());
            }
        } catch (Exception e) {
